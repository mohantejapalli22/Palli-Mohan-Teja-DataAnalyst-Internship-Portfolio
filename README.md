# Palli-Mohan-Teja-DataAnalyst-Internship-Portfolio
# Data Analytics Internship Portfolio – ApexPlanet

## 📌 Overview
This repository serves as the **central hub** for my 60‑day Data Analytics Internship at ApexPlanet Software Pvt. Ltd.  
It consolidates all tasks, deliverables, and reflections into one professional portfolio.

---

## 🗂️ Repository Structure
- **Task 1 – Data Wrangling**  
  - [GitHub Repo](https://github.com/mohantejapalli22/ApexPlanet-Data-Analytics---Task1.git)  
  - [LinkedIn Video]((https://www.linkedin.com/posts/mohan-teja-palli-82b158367_dataanalytics-python-pandas-activity-7463121489211396098-V6yH?utm_source=share&utm_medium=member_android&rcm=ACoAAFr4C6MBj3NoCOJarys006eqks1jFF1-lNc))  
  - Deliverables: Data dictionary, cleaning scripts, cleaned dataset  

- **Task 2 – Exploratory Data Analysis (EDA) & Business Intelligence**  
  - [GitHub Repo/Folder Link]([INSERT_LINK](https://github.com/mohantejapalli22/ApexPlanet-Data-Analytics---Task2.git))  
  - [LinkedIn Video]((https://www.linkedin.com/posts/mohan-teja-palli-82b158367_dataanalytics-eda-businessintelligence-activity-7465707751676022784-wyea?utm_source=share&utm_medium=member_android&rcm=ACoAAFr4C6MBj3NoCOJarys006eqks1jFF1-lNc))  
  - Deliverables: EDA report, SQL queries, static dashboard mock‑up  

- **Task 3 – Deep‑Dive Analysis & Interactive Dashboarding**  
  - [GitHub Repo/Folder Link]((https://github.com/mohantejapalli22/ApexPlanet-Data-Analytics---Task3.git))  
  - [LinkedIn Video](https://www.linkedin.com/posts/mohan-teja-palli-82b158367_dataanalytics-powerbi-python-activity-7470523582977056768-TuhN?utm_source=share&utm_medium=member_android&rcm=ACoAAFr4C6MBj3NoCOJarys006eqks1jFF1-lNc)  
  - Deliverables: Deep‑dive report, interactive dashboard (Power BI/Tableau)  

- **Task 4 – Data Storytelling & Statistical Validation**  
  - [GitHub Repo/Folder Link]((https://github.com/mohantejapalli22/ApexPlanet-Data-Analytics---Task4.git))  
  - [LinkedIn Video](https://www.linkedin.com/posts/mohan-teja-palli-82b158367_dataanalytics-datastorytelling-statisticalanalysis-ugcPost-7481957974589239296-jZra/?utm_source=share&utm_medium=member_android&rcm=ACoAAFr4C6MBj3NoCOJarys006eqks1jFF1-lNc)  
  - Deliverables: Final presentation deck, hypothesis testing summary  
    
---

## 📊 Key Skills Demonstrated
- **Data Wrangling:** Python, Pandas, SQL  
- **Exploratory Analysis:** Descriptive statistics, visualizations, SQL queries  
- **Business Intelligence:** KPI definition, dashboarding (Power BI/Tableau)  
- **Storytelling & Validation:** Presentation design, hypothesis testing (T‑test, Chi‑square)  
- **Portfolio Building:** GitHub organization, professional reflection, LinkedIn engagement  

---

## 🎯 Professional Reflection
This internship allowed me to:
- Practice **end‑to‑end analytics workflow** from raw data to insights.  
- Build **interactive dashboards** and communicate findings effectively.  
- Apply **statistical validation** to support business decisions.  
- Create a **professional portfolio** showcasing technical and storytelling skills.  

---
🙏 Acknowledgement

I sincerely thank ApexPlanet Software Pvt. Ltd. for providing this valuable internship opportunity and helping me strengthen my practical knowledge in Data Analytics.

---

## ✅ Conclusion
This portfolio demonstrates my ability to handle real‑world data analytics projects, communicate insights, and present them professionally. It serves as a showcase of my internship journey and the skills I’ve developed.

Submitted by: Palli Mohan Teja
